...
class insert:
    def __init__(self):
        self.connection=mysql.connector.connect(
            user="root",
            password='Abdulmalik8@',
            database="bank_manegment"
        )
    #create cursor
    ...
    def account_details(self,acco,tranid,amount,closbala,transactiontype):
        cur=self.connection.cursor()
        cur.execute(f"insert into account_details values({acco},{tranid},{amount},{closbala},'{transactiontype}')")
        self.connection.commit()
        print('-----details are saved--------')
